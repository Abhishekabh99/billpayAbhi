...
class _OtpScreenState extends State<OtpScreen> {
  ...
  onVerifyButtonClick() async {
    //validating OTP lemgth entered by user
    if (_otpPinFieldController.currentState?.text.length != 6) {
      CherryToast.warning(
    ...
    final response = await AuthApi.verifyOTP(
        widget.phoneNumber, _otpPinFieldController.currentState?.text ?? "");
    if (response?.success == true) {
      if (response!.token != null && response.token!.isNotEmpty) {
        ...
        context.push(AuthPath.chooseLang, extra: {
